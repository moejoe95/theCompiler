	...
	if (lhs->type != MCC_AST_EXPRESSION_TYPE_LITERAL) {
		if (lhs->type == MCC_AST_EXPRESSION_TYPE_IDENTIFIER) {
			...
			if (lhs->array_access_exp->type == MCC_AST_EXPRESSION_TYPE_LITERAL ||
				...
				char *tmp = generate_ir_entity(head, lhs->array_access_exp);
				sprintf(arg2, "%s", tmp);
				free(tmp);
	...
	if (rhs->type != MCC_AST_EXPRESSION_TYPE_LITERAL) {
		if (rhs->type == MCC_AST_EXPRESSION_TYPE_IDENTIFIER) {
			...
			if (rhs->array_access_exp->type == MCC_AST_EXPRESSION_TYPE_LITERAL ||
				...
				char *tmp = generate_ir_entity(head, rhs->array_access_exp);
				sprintf(arg2, "%s", tmp);
				free(tmp);
				char *table_arg = lookup_table_args(head, arg2, NULL, -1);
				if (strcmp(table_arg, "0") != 0)
                                         ...
                                         enum ir_table_operation_type type)
{
	...
	if (un_expr->rhs->type == MCC_AST_EXPRESSION_TYPE_LITERAL) {
		...
		if (un_expr->rhs->type == MCC_AST_EXPRESSION_TYPE_ARRAY_ACCESS) {
			entity1 = generate_ir_array_access(head, un_expr->rhs);
		} else {
			entity1 =
			    lookup_table_args(head, un_expr->rhs->identifier->name, NULL, un_expr->expression_type);
		}
	}
	...
	if (expr) { // needed if function returns void (nothing)
		...
		if (expr->type != MCC_AST_EXPRESSION_TYPE_LITERAL && expr->type != MCC_AST_EXPRESSION_TYPE_IDENTIFIER) {
			if (expr->type == MCC_AST_EXPRESSION_TYPE_ARRAY_ACCESS) {
				...
				char *value2 = generate_ir_entity(head, expr->array_access_exp);
				generate_ir_table_line(head, strdup(value), strdup(value2), MCC_IR_TABLE_RETURN, -1, -1,
				                       -1);
			} else {
				sprintf(value, "(%d)", head->index);
