...
void create_asm_assignment(FILE *out, struct mcc_ir_line *line, struct mcc_asm_head *head)
{
        ...
        if (line->memory_size == 2) { // single float values
                if (line->arg2[0] != '(') {
                        head->offset = head->offset -4;
                        char* datasec = add_asm_float(line->arg2, line->index, head);
                        push_on_stack(line, head);
                        print_asm_instruction_load_float(out, MCC_ASM_INSTRUCTION_FLDS, datasec);
                        print_asm_instruction_store_float(out, MCC_ASM_INSTRUCTION_FSTPS, MCC_ASM_REGISTER_EBP, head->offset);
                } else {
                        char label[64] = {0};
                        sprintf(label, "(%d)", line->index);
                        int stack_pos = find_stack_position(line->arg2, head->stack);
                        int stack_pos_var = find_stack_position(line->arg1, head->stack);
                        if (stack_pos != -1) {
                                print_asm_instruction_reg(out, MCC_ASM_INSTRUCTION_MOVL, MCC_ASM_REGISTER_EBP,
                                                          stack_pos, MCC_ASM_REGISTER_EAX, 0);
                                print_asm_instruction_reg(out, MCC_ASM_INSTRUCTION_MOVL, MCC_ASM_REGISTER_EAX, 0,
                                                          MCC_ASM_REGISTER_EBP, stack_pos_var);
                                push_on_stack(line, head);
                        } else
